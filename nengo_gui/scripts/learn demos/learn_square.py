...
# This demo shows you how to construct a network containing an ensemble which
# learns how to decode the square of its value.

# ## Step 1: Create the Model

# This network consists of an ensemble which represents the input, ('A'),
# an ensemble which learns to represent the square, ('A_squared'), and an
# ensemble which represents the error between A_squared and the actual square,
# ('error').
...
with model:
    ...
    nengo.Connection(A, error, function=lambda x: x**2)

    # Create an input node that steps between -1 and 1
    input_node = nengo.Node(
        output=lambda t: int(6*t/5)/3.0 % 2 - 1, label="input")
