...
class NoTransform(Transform):
    """A noop transform (representing the absence of any transform operations).
    ...
    def sample(self, rng=np.random):
        ...
        ------
        TypeError
            There is nothing to sample for Null transforms, so it is an error
            if this is called.
        """
        raise TypeError("Cannot sample a Null transform")
