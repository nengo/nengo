...
class Builder(object):
    ...
    builds = lambda cls: nengo.utils.register(
    ...
    def __init__(self, network, dt):
        ...
        seed = nengo.utils.random_maxint(np.random) if network.seed is None \
            else network.seed
    ...
    def _build_ensemble(self, ens):
        ...
        if ens.encoders is None:
            if isinstance(ens.neurons, nengo.nonlinearities.Direct):
    ...
    def _build_pyfunc(self, pyfn):
        if pyfn.n_in > 0:
            self.model.sig_in[pyfn] = Signal(np.zeros(pyfn.n_in),
                                             name="%s.input" % pyfn.label)
            self.model.operators.append(Reset(self.model.sig_in[pyfn]))
        self.model.sig_out[pyfn] = Signal(np.zeros(pyfn.n_out),
                                          name="%s.output" % pyfn.label)
        self.model.operators.append(
            SimPyFunc(output=self.model.sig_out[pyfn],
                      J=self.model.sig_in[pyfn] if pyfn.n_in > 0 else None,
                      fn=pyfn.fn,
                      n_args=pyfn.n_args))
    ...
    def _build_node(self, node):
        ...
        if node.output is None:
            ...
            pyfn = nengo.nonlinearities.PythonFunction(
                fn=node.output,
                n_in=node.size_in,
                n_out=node.size_out,
            ...
            if node.size_in > 0:
                self.model.operators.append(DotInc(
                    ...
                    Signal(1.0, name="1"),
                    self.model.sig_in[pyfn],
                    tag="%s input" % node.label))
            self.model.sig_out[node] = self.model.sig_out[pyfn]
    ...
    def _build_probe(self, probe, dimensions):
        ...
        self.model.sig_in[probe] = probe_signal
        # Reset input signal to 0 each timestep
        self.model.operators.append(Reset(probe_signal))
    ...
    def _filtered_signal(self, signal, pstc):
        ...
        o_coef, n_coef = self._filter_coefs(pstc=pstc, dt=self.model.dt)
    ...
    def _direct_pyfunc(self, input_signal, function, n_out, label):
        pyfunc = nengo.nonlinearities.PythonFunction(
            fn=function, n_in=input_signal.size, n_out=n_out, label=label)
        self._build_pyfunc(pyfunc)
        self.model.operators.append(DotInc(
            input_signal,
            Signal(1.0, name="1"),
            self.model.sig_in[pyfunc],
            tag="%s input" % label))
        return pyfunc

    @builds(nengo.objects.Connection)  # noqa
    def _build_connection(self, conn):
        ...
        if (isinstance(conn.pre, nengo.objects.Ensemble)
                and isinstance(conn.pre.neurons, nengo.nonlinearities.Direct)):
            ...
            if conn.function is None:
                ...
                pyfunc = self._direct_pyfunc(
                    ...
                    conn.dimensions,
                    conn.label)
                signal = self.model.sig_out[pyfunc]
            ...
            activities = self.model.dt * conn.pre.neurons.rates(
            ...
            if conn.filter is not None and conn.filter > self.model.dt:
                o_coef, n_coef = self._filter_coefs(
                    pstc=conn.filter, dt=self.model.dt)
            ...
            signal = self._filtered_signal(
                self.model.sig_in[conn], conn.filter)
        else:
            # 4. Direct connection
            signal = self.model.sig_in[conn]
        ...
        for probe in conn.probes["signal"]:
            self.build(probe, dimensions=self.sig_out[conn].size)

    @builds(nengo.nonlinearities.Direct)
    ...
    @builds(nengo.nonlinearities.LIFRate)
    ...
    @builds(nengo.nonlinearities.LIF)
