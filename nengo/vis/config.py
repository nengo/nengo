...
class Config(nengo.config.Config):
    ...
    def __init__(self):
        ...
        for klass in [nengo.Ensemble, nengo.Node]:
            ...
            self[klass].set_param('pos', nengo.params.Parameter((0, 0)))
            self[klass].set_param('scale', nengo.params.Parameter(1))
            self[klass].set_param('size', nengo.params.Parameter((50, 50)))

        self.configures(nengo.Network)
        self[nengo.Network].set_param('scale', nengo.params.Parameter(1.))
        self[nengo.Network].set_param('offset', nengo.params.Parameter((0,0)))
        self[nengo.Network].set_param('pos', nengo.params.Parameter((-50, -50)))
        self[nengo.Network].set_param('size', nengo.params.Parameter((100, 100))) #width/height
