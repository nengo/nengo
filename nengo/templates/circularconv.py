...
class CircularConvolution(object):
    ...
    def __init__(self, model, A, B,
        ...
        for ii in range(D//2):
            ...
            model.encoder(self.A_Fourier[ii, 0], AA)
            model.encoder(self.A_Fourier[ii, 1], AA)
            model.encoder(self.A_Fourier[ii, 0], AB)
            model.encoder(self.B_Fourier[ii, 1], AB)
            model.encoder(self.B_Fourier[ii, 0], BA)
            model.encoder(self.A_Fourier[ii, 1], BA)
            model.encoder(self.B_Fourier[ii, 0], BB)
            model.encoder(self.B_Fourier[ii, 1], BB)
            
            model.decoder(AA, self.AB_prods[ii, 0])
            model.decoder(AB, self.AB_prods[ii, 1])
            model.decoder(BA, self.AB_prods[ii, 2])
            model.decoder(BB, self.AB_prods[ii, 3])
