...
def test_fixed():
    ...
    assert np.allclose(model.input.input_nodes["buffer1"].output, vocab1.parse("A").v)
    assert np.allclose(model.input.input_nodes["buffer2"].output, vocab2.parse("B").v)
...
def test_time_varying():
    ...
    assert np.allclose(
        model.input.input_nodes["buffer"].output(t=0), vocab.parse("A").v
    )
    assert np.allclose(
        model.input.input_nodes["buffer"].output(t=0.1), vocab.parse("B").v
    )
    assert np.allclose(model.input.input_nodes["buffer"].output(t=0.2), np.zeros(16))
