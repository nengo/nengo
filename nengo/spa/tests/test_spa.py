...
def test_spa_basic(Simulator):  # noqa: C901
    ...
    class SpaTestBasic(spa.SPA):
        class Rules:
            def a():
                ...
                effect(state='B')  # noqa: F821

            def b():
                match(state='B')  # noqa: F821
                effect(state='C')  # noqa: F821

            def c():
                match(state='C')  # noqa: F821
                effect(state='D')  # noqa: F821

            def d():
                match(state='D')  # noqa: F821
                effect(state='E')  # noqa: F821

            def e():
                match(state='E')  # noqa: F821
                effect(state='A')  # noqa: F821
    ...
    with Plotter(Simulator) as plt:
        ...
        plt.savefig('test_spa.test_spa_basic.pdf')
