...
def test_dts(Simulator):
    ...
    model = nengo.Network('test_probe_dts', seed=2891)
    with model:
        probes = []
...
def test_large(Simulator):
    ...
    model = nengo.Network('test_large_probes', seed=3249)
    with model:
        probes = []
        for i in range(n):
            xi = nengo.Node(label='x%d' % i, output=input_fn)
...
def test_defaults(Simulator):
    ...
    model = nengo.Network('test_defaults')
    with model:
