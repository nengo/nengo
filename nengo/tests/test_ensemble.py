...
from nengo.utils.processes import MarkovProcess, SampledProcess
...
def test_noise_copies_ok(Simulator, nl_nodirect, seed, plt):
    ...
    """
    # Use a MarkovProcess since it has per-ensemble state
    process = MarkovProcess(Choice([0.1]), initial_state=np.array([-1]))
    with nengo.Network(seed=seed) as model:
        inp, gain, bias = 1, 5, 2
        model.config[nengo.Ensemble].neuron_type = nl_nodirect()
        model.config[nengo.Ensemble].encoders = Choice([[1]])
