...
def test_remove_passthrough():
    ...
    with model:
        ...
        nengo.Connection(a.output, a.input, filter=0.01, transform=np.ones((D,D)))
        nengo.Connection(b.output, output, filter=0.01)

    objs, conns = nengo.build_utils.remove_passthrough_nodes(model.objs, model.connections)
...
def test_passthrough_errors():
    ...
    with pytest.raises(NotImplementedError):
        nengo.build_utils.remove_passthrough_nodes(model.objs, model.connections)
